        ...
        settings_keyboard = {
            ...
            "buttons": [
                ...
                [get_button(label="Назад", color="secondary"),
                 get_button(label="Сменить группу", color="secondary"),
        ...
        if (event.type == VkBotEventType.MESSAGE_NEW) and (event.object.peer_id == event.object.from_id):
            ...
            # main_keyboard - working with commands of this level
                ...
                if presence_user != []:
                    write_msg(user_id=event.object.peer_id,
                              ...
                              message="Это раздел настроек - если плитка зелёная, то функция включена, если же красная, то выключена. Чтобы изменить один из параметров уведомлений, следует нажать на необходимую плитку🤗")
                    write_msg(user_id=event.object.peer_id,
                              keyboard=create_settings_keyboard(keyboard_user_id=f"id{event.object.peer_id}"),
                              message="Кстати, время, когда будет происходить автоматическая рассылка расписания, можно отредактировать🤫\nПросто введи нужное тебе значение, например: 08:00")
                    write_msg(user_id=event.object.peer_id,
                              keyboard=create_settings_keyboard(keyboard_user_id=f"id{event.object.peer_id}"),
                              message="А ещё, если у тебя сменилась группа, ты можешь получить некоторую инструкцию для редактирования своих данных, нажав на соответствующую плитку😉")
                else:
                    write_msg(user_id=event.object.peer_id, keyboard=main_keyboard,
            ...
            # settings_keyboard - working with commands of this level
                ...
                presence_user = UserSearcher.searching_user_in_database(
                    database_source="workWithUsersDatabase/UsersDatabase.txt", user_id=f"id{event.object.peer_id}")
                if presence_user != []:
                    if str("" if len(event.object.text) != 4 else "0") + str(event.object.text) != presence_user[7]:
                ...
                if presence_user != []:
                    if event.object.text.upper() != presence_user[3]:
                        if event.object.text.upper() in list_of_groups_in_the_class("8class"):
                            ...
                            write_msg(user_id=event.object.peer_id, keyboard=main_keyboard,
                                      message=f"Поздравляю! Смена группы индивидуального расписания с {presence_user[3]} ({presence_user[2]}) на {event.object.text.upper()} (8class) прошла успешно✅")
                            ...
                            write_msg(user_id=event.object.peer_id, keyboard=main_keyboard,
                                      message=f"Поздравляю! Смена группы индивидуального расписания с {presence_user[3]} ({presence_user[2]}) на {event.object.text.upper()} (9class) прошла успешно✅")
                            ...
                            write_msg(user_id=event.object.peer_id, keyboard=main_keyboard,
                                      message=f"Поздравляю! Смена группы индивидуального расписания с {presence_user[3]} ({presence_user[2]}) на {event.object.text.upper()} (10class) прошла успешно✅")
                            ...
                            write_msg(user_id=event.object.peer_id, keyboard=main_keyboard,
                                      message=f"Поздравляю! Смена группы индивидуального расписания с {presence_user[3]} ({presence_user[2]}) на {event.object.text.upper()} (11class) прошла успешно✅")
                            ...
                            write_msg(user_id=event.object.peer_id, keyboard=main_keyboard,
                                      message=f"Поздравляю! Смена группы индивидуального расписания с {presence_user[3]} ({presence_user[2]}) на ГОСТЬ (GUESTS) прошла успешно✅")
                        ...
                        write_msg(user_id=event.object.peer_id, keyboard=main_keyboard,
                                  message="Да-да, всё внесено верно - ты уже и так записан с этими же данными в базе. Если есть какие-то вопросы, то пиши в беседу, прикреплённую к сообществу🗿\nhttps://vk.me/join/FhSVyJp7fYT0fM805_KTHNWPctDNa79JGsI=")
                else:
                    if event.object.text.upper() in list_of_groups_in_the_class("8class"):
            ...
            # links to Zoom tables depending on the class number and other parameters
                ...
                if presence_user != []:
                    if presence_user[3] in list_of_groups_in_the_class("8class"):
                        write_msg(user_id=event.object.peer_id, keyboard=main_keyboard,
                                  message=f"Старайся не отключать камеру с микрофоном на уроке, ведь это позволит провести занятие качественнее🤪\n{BotConfig.links_to_zoom[0]}")
